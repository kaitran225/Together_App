...
export function AppLayout() {
  ...
  const navItems = [
    { to: '/dashboard', labelKey: 'nav.admin.dashboard' },
    { to: '/study-rooms', labelKey: 'nav.studyRooms' },
    { to: '/teams', labelKey: 'nav.projects' },
    { to: '/meet-ai', labelKey: 'nav.aiTutor' },
  ] as const
    ...
    <div className="min-h-screen flex flex-col bg-transparent">
      <header className="bg-[var(--color-surface)] border-b border-white/10 px-4 py-3 md:px-8 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Input
            placeholder={t('common.searchPlatform')}
            className="max-w-64 min-h-[40px] py-2"
            aria-label={t('common.search')}
          />
          <Link to="/notifications">
            <Button variant="ghost" size="sm" aria-label={t('nav.notifications')} className="p-2">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
            </Button>
          </Link>
          <Link to="/profile">
            <Button variant="ghost" size="sm" aria-label={t('common.profile')} className="p-2">
      ...
      <footer className="bg-[var(--color-surface)] border-t border-white/10 px-4 py-3 mt-auto">
        <div className="max-w-[1200px] mx-auto flex flex-wrap justify-between items-center gap-4">
          <div className="flex items-center gap-4">
            <div>
              <p className="label-study text-[10px]">{t('layout.currentStreak')}</p>
              <p className="text-sm font-bold">{t('layout.daysCount', { count: 12 })}</p>
            </div>
            <div className="w-px h-8 bg-white/10" />
            <div className="w-64">
              <div className="flex justify-between text-[10px] font-bold uppercase">
                <span>{t('layout.level', { level: 14 })}</span>
          ...
          <div className="flex gap-6">
            <Link to="/quests"><span className="text-[10px] font-bold uppercase">{t('layout.quests')}</span></Link>
            <Link to="/rank"><span className="text-[10px] font-bold uppercase">{t('layout.rank')}</span></Link>
          </div>
        </div>
      </footer>
