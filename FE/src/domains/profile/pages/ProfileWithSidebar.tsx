...
export default function ProfileWithSidebar() {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.save(`${user?.fullName || t('profile.defaultUser')}_CV.pdf`)
    } catch (e) {
  ...
  const handleSaveProfile = async () => {
    if (!editForm.fullName.trim()) {
      setEditError(t('profile.fullNameRequired'))
      return
    }
    setIsSaving(true)
    setEditError('')
    ...
    if (res.ok) {
      ...
      setEditError(res.error || t('profile.updateError'))
    }
  }
  ...
  const handleAvatarUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setEditError(t('profile.imageSizeError'));
      return;
    }
    ...
    reader.onerror = () => {
      setEditError(t('profile.imageReadError'));
    };
  ...
  const displayName = user?.fullName || user?.email?.split('@')[0] || t('profile.defaultUser')
  ...
  const monthLabels = [
    t('profile.jan'), t('profile.feb'), t('profile.mar'), t('profile.apr'),
    t('profile.may'), t('profile.jun'), t('profile.jul'), t('profile.aug'),
    t('profile.sep'), t('profile.oct'), t('profile.nov'), t('profile.dec'),
  ]
  const weekdayLabels = [
    t('profile.mon'), t('profile.tue'), t('profile.wed'), t('profile.thu'),
    t('profile.fri'), t('profile.sat'), t('profile.sun'),
  ]
    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b-2 border-neutral-200">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-600">{t('profile.planLevel', { plan: user?.planType ?? 'FREE', level: userLevel })}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[40fr_40fr_20fr] gap-6 items-start">
        ...
        <div className="flex flex-col gap-6 min-w-0">
          <Card className="p-5 border-2 border-neutral-200" heading={t('profile.levelProgress')}>
            <Progress value={currentLevelXp} max={xpTarget} label={<><span>{t('profile.xpToLevel', { current: currentLevelXp.toLocaleString(), target: xpTarget.toLocaleString(), level: userLevel + 1 })}</span><span>{Math.round(levelProgress)}%</span></>} />
          </Card>
          <Card className="p-5 border-2 border-neutral-200" heading={t('profile.completedWork')}>
            {completedTasks.length === 0 ? (
              <p className="text-xs text-neutral-500">{t('profile.noCompletedTasks')}</p>
            ) : (
              <ul className="space-y-1.5">
                {completedTasks.slice(0, 5).map((task) => (
                  <li key={task.id} className="flex items-center justify-between gap-2 text-sm">
                    <span className="text-neutral-900 truncate font-medium">{task.title}</span>
            ...
            <Link to="/dashboard" className="inline-block mt-3 text-xs font-semibold text-neutral-700 hover:text-neutral-900">{t('profile.viewFull')}</Link>
          </Card>
          <Card className="p-5 border-2 border-neutral-200" heading={t('profile.achievements')}>
              ...
              {achievements.map((ach) => {
                ...
                return (
                  <div 
                    ...
                    title={`${ach.displayName}: ${ach.description} (${unlocked ? t('profile.unlocked') : t('profile.notUnlocked')})`}
            ...
            <p className="text-xs text-neutral-500 mt-2">
              {t('profile.unlockedCount', { unlocked: achievements.filter(a => a.isUnlocked).length, total: achievements.length })}
            </p>
          </Card>
  ...
  <div className="relative z-10">
    ...
    <div className="flex items-center gap-3 mb-5">
      ...
      <h3 className="text-xl font-bold text-primary tracking-tight">{t('profile.learningGoals')}</h3>
    ...
    <div className="flex flex-col gap-2 mb-4">
      {(!user?.learningGoals || user.learningGoals.length === 0) ? (
        <p className="text-sm text-neutral-500 italic py-2">{t('profile.noLearningGoals')}</p>
      ) : (
    ...
    {showAddGoalInput ? (
      <div className="flex items-center gap-3 mt-2">
        ...
        />
        <Button size="sm" onClick={handleAddGoal} className="bg-primary hover:bg-primary/90 text-white shadow-sm hover:shadow transition-all px-5">{t('common.save')}</Button>
        <Button size="sm" variant="ghost" onClick={() => setShowAddGoalInput(false)} className="text-neutral-500 hover:text-neutral-700">{t('common.cancel')}</Button>
      </div>
    ) : (
      <Button 
        variant="secondary" 
      ...
      >
        ...
        {/* Column 2 (40%) */}
        <div className="flex flex-col gap-6 min-w-0">
          <Card className="p-5 border-2 border-neutral-200" heading={t('profile.weeklySchedule')}>
            ...
            <Link to="/calendar" className="inline-block mt-3 text-xs font-semibold text-neutral-700 hover:text-neutral-900">{t('profile.viewFull')}</Link>
          </Card>
          <Card className="p-5 border-2 border-neutral-200" heading={t('profile.studyTimeByMonth')}>
            <div className="flex items-end gap-0.5 h-20">
              {MONTHLY_HOURS.map((h, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-0.5 min-w-0">
                  <span className="text-[8px] font-medium text-neutral-500 truncate w-full text-center">{monthLabels[i]}</span>
          ...
          <Card className="p-5 border-2 border-neutral-200" heading={t('profile.myQuizzes')}>
            <ul className="space-y-2">
              {quizzesList.slice(0, 5).map((q) => (
                <li key={q.quizId} className="flex items-center justify-between gap-2 text-sm py-1.5 border-b border-neutral-100 last:border-0">
                  <span className="text-neutral-900 truncate font-medium">{q.title || t('profile.quizFallback', { id: q.quizId })}</span>
                  <span className="text-neutral-500 text-xs shrink-0">{t('profile.questionsShort', { count: q.questionCount })}</span>
                  <span className="font-bold text-neutral-900 w-8 text-right shrink-0">{q.difficulty?.charAt(0).toUpperCase() || 'M'}</span>
                </li>
              ))}
              {quizzesList.length === 0 && (
                <li className="text-neutral-500 text-sm italic">{t('profile.noQuizzes')}</li>
              )}
            </ul>
          </Card>
          <Card className="p-5 border-2 border-neutral-200" heading={t('profile.notes')}>
            {notesList.length === 0 ? (
              <p className="text-xs text-neutral-500">{t('profile.noNotes')}</p>
            ) : (
              <ul className="space-y-2">
        ...
        {/* Column 3 (20%) */}
        <div className="flex flex-col gap-6 min-w-0">
          <Card className="p-5 border-2 border-neutral-200" heading={t('profile.skills')}>
            <div className="flex flex-wrap gap-2 mb-3">
              {(!user?.skills || user.skills.length === 0) ? (
                <p className="text-xs text-neutral-500">{t('profile.noSkills')}</p>
              ) : (
                user.skills.map((s) => (
            ...
            {showAddSkillInput ? (
              <div className="flex items-center gap-2 mt-2">
                ...
                />
                <Button size="sm" onClick={handleAddSkill}>{t('common.save')}</Button>
                <Button size="sm" variant="secondary" onClick={() => setShowAddSkillInput(false)}>{t('common.cancel')}</Button>
              </div>
            ) : (
              <Button variant="ghost" size="sm" onClick={() => setShowAddSkillInput(true)}>{t('profile.addSkill')}</Button>
            )}
          </Card>
          <Card className="p-5 border-2 border-neutral-200" heading={t('profile.statistics')}>
            <ul className="space-y-2 text-sm">
              <li className="flex justify-between"><span className="text-neutral-600">{t('profile.totalStudyTime')}</span><strong className="text-neutral-900">{Math.round(totalExp / 60)}h</strong></li>
              <li className="flex justify-between"><span className="text-neutral-600">{t('profile.tasksCompleted')}</span><strong className="text-neutral-900">{completedTasks.length}</strong></li>
              <li className="flex justify-between"><span className="text-neutral-600">{t('profile.notesCreated')}</span><strong className="text-neutral-900">{notesList.length}</strong></li>
              <li className="flex justify-between"><span className="text-neutral-600">{t('profile.streak')}</span><strong className="text-neutral-900">{t('profile.streakDays', { count: user?.streak ?? 0 })}</strong></li>
            </ul>
          </Card>
          <Card className="p-5 border-2 border-neutral-200" heading={t('profile.account')}>
            {user && (
            ...
            <Button variant="secondary" size="sm" className="w-full" onClick={logout}>{t('common.logout')}</Button>
          </Card>
          <Card className="p-5 border-2 border-neutral-200" heading={t('profile.nextReward')}>
            <div className="mb-2">
              <div className="h-2 w-full bg-neutral-200 rounded-full overflow-hidden">
                <div className="h-full bg-accent rounded-full" style={{ width: `${levelProgressPercent}%` }} />
              </div>
              <p className="text-[10px] text-neutral-500 mt-1">{t('profile.levelProgressShort', { percent: levelProgressPercent, level: userLevel + 1 })}</p>
            </div>
          </Card>
        </div>
      </div>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 mt-2">
            <span className="text-sm font-semibold text-neutral-700">{t('profile.avatar')}</span>
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-full bg-neutral-200 border border-neutral-300 flex items-center justify-center overflow-hidden shrink-0">
                {editForm.avatarUrl ? (
                  <img src={editForm.avatarUrl} alt={t('profile.preview')} className="w-full h-full object-cover" />
                ) : (
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-neutral-500 mt-1">{t('profile.avatarHint')}</p>
              </div>
            </div>
            
            <details className="mt-2 text-sm text-neutral-500">
              <summary className="cursor-pointer font-medium hover:text-neutral-700">{t('profile.advancedAvatarUrl')}</summary>
              <div className="mt-2">
                <Input
                  label=""
      ...
      >
        <div className="flex flex-col gap-4 py-2">
          <p className="text-sm text-neutral-600">{t('profile.shareDescription')}</p>
          ...
          {shareCopied && <p className="text-sm text-green-600 text-center font-medium mt-2">{t('profile.linkCopied')}</p>}
        </div>
      </Modal>
      ...
      <div id="pdf-export-content" className="hidden bg-white w-[800px] p-10 text-neutral-900 absolute top-0 left-0 -z-50" style={{ fontFamily: 'sans-serif' }}>
        <div className="flex items-center gap-6 mb-10 border-b pb-6">
          <div className="w-24 h-24 rounded-full bg-neutral-200 overflow-hidden shrink-0">
            {user?.avatarUrl ? <img src={user.avatarUrl} alt={t('profile.avatar')} className="w-full h-full object-cover" crossOrigin="anonymous" /> : <div className="w-full h-full flex items-center justify-center text-3xl font-bold bg-[#6C5CE7] text-white">{user?.fullName?.charAt(0) || 'U'}</div>}
          </div>
          <div>
            <h1 className="text-4xl font-bold uppercase tracking-tight">{user?.fullName || t('profile.defaultUser')}</h1>
            <p className="text-lg text-neutral-600 mt-1">{t('profile.planLevel', { plan: user?.planType ?? 'FREE', level: userLevel })}</p>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-8">
          <div>
            <h2 className="text-xl font-bold mb-4 uppercase text-[#6C5CE7] border-b pb-2 border-neutral-200">{t('profile.skills')}</h2>
            <div className="flex flex-wrap gap-2">
              {user?.skills?.map(s => (
                <span key={s} className="px-3 py-1 bg-neutral-100 rounded-full text-sm font-medium">{s}</span>
              ))}
              {(!user?.skills || user.skills.length === 0) && <p className="text-neutral-500 italic">{t('profile.noSkillsAdded')}</p>}
            </div>
          </div>
          
          <div>
            <h2 className="text-xl font-bold mb-4 uppercase text-[#6C5CE7] border-b pb-2 border-neutral-200">{t('profile.statistics')}</h2>
            <ul className="space-y-3">
              <li className="flex justify-between"><span className="text-neutral-600">{t('profile.totalStudyTime')}</span><strong>{Math.round(totalExp / 60)}h</strong></li>
              <li className="flex justify-between"><span className="text-neutral-600">{t('profile.tasksCompleted')}</span><strong>{completedTasks.length}</strong></li>
              <li className="flex justify-between"><span className="text-neutral-600">{t('profile.notesCreated')}</span><strong>{notesList.length}</strong></li>
              <li className="flex justify-between"><span className="text-neutral-600">{t('profile.streak')}</span><strong>{t('profile.streakDays', { count: user?.streak ?? 0 })}</strong></li>
            </ul>
          </div>

          <div className="col-span-2 mt-4">
            <h2 className="text-xl font-bold mb-4 uppercase text-[#6C5CE7] border-b pb-2 border-neutral-200">{t('profile.learningGoals')}</h2>
            <ul className="list-disc pl-5 space-y-2">
              {user?.learningGoals?.map(g => (
                <li key={g} className="text-neutral-700">{g}</li>
              ))}
              {(!user?.learningGoals || user.learningGoals.length === 0) && <p className="text-neutral-500 italic">{t('profile.noGoalsAdded')}</p>}
            </ul>
          </div>
          
          <div className="col-span-2 mt-4">
            <h2 className="text-xl font-bold mb-4 uppercase text-[#6C5CE7] border-b pb-2 border-neutral-200">{t('profile.achievements')}</h2>
